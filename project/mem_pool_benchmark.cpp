...
//   - MemPool<T>    — our custom free-list pool allocator
...
//
// Why MemPool wins for this kind of workload:
//   - MemPool stores elements contiguously in a pre-allocated vector, so all
//     slots are cache-friendly and there is zero system-allocator overhead.
...
static uint64_t bench_pool_allocate(MemPool<Order>& pool, std::vector<Order*>& ptrs, std::size_t n) {
...
static uint64_t bench_pool_deallocate(MemPool<Order>& pool, std::vector<Order*>& ptrs, std::size_t n) {
    const auto start = Common::rdtsc();
    for (std::size_t i = 0; i < n; ++i) {
...
static uint64_t bench_pool_dealloc_random(MemPool<Order>& pool, std::vector<Order*>& ptrs,
...
static uint64_t bench_pool_churn(MemPool<Order>& pool, std::size_t ops, std::mt19937& rng) {
    // Pre-fill pool to ~50%.
...
static uint64_t bench_pool_allocate_cold(MemPool<Order>& pool, std::vector<Order*>& ptrs) {
...
static void run_benchmark(std::size_t n) {
    ...
    for (std::size_t i = 0; i < cold_n; ++i) std::free(malloc_cold_ptrs[i]);

    // ---- results ----
    std::cout << "\n=== MemPool Benchmark (N=" << n << ") ===\n\n";
